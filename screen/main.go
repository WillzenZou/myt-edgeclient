...
static Decoder* create_decoder() {
    ...
    for (int i = 0; hw_decoders[i]; i++) {
        ...
        else if (strstr(hw_decoders[i], "videotoolbox")) type = AV_HWDEVICE_TYPE_VIDEOTOOLBOX;
        else if (strstr(hw_decoders[i], "h264_videotoolbox")) type = AV_HWDEVICE_TYPE_VIDEOTOOLBOX;
        else if (strstr(hw_decoders[i], "h264")) type = AV_HWDEVICE_TYPE_VIDEOTOOLBOX;
...
var main_content *fyne.Container
...
func videoStreamCallback(rot C.int, data unsafe.Pointer, length C.int) {
	...
	case videoStreamChan <- VideoFrame{
		...
		if currentRotation != int(rot) {
			...
			//fmt.Println("2大小改变", newWidth, newHeight)
			main_content.Move(fyne.NewPos(0, 0))
			interactiveImg.Resize(fyne.NewSize(newWidth, newHeight))
			interactiveImg.Refresh()
			touchSession.windowSize = fyne.NewSize(newWidth, newHeight)
			touchSession.lock.Unlock()
			//projectionWindow
		}
	default:
		// 丢弃最旧帧
...
func main() {
	...
	// 初始化触摸会话
    touchSession = NewTouchSession(*height, *width) // 注意实际分辨率与旋转方向

    // 获取端口并连接
    //touchSession.handle = sdkHandle
    err := touchSession.checkHandle(*ipaddr, *port, *bakport)
    if err != nil {
		showError("连接失败")
		return
    }
	...
	// 绑定事件处理
    setupInputHandlers(interactiveImg, touchSession)
	...
	// 启动视频流
	startVideoStream(C.long(touchSession.handle), 1280, 720, 2000)

	go decodeAndDisplayVideo(projectionImg, stopChan, &wg)

	// fixed: macos initial size
	main_content = container.NewStack(interactiveImg,container.NewGridWrap(fyne.NewSize(200, 200), widget.NewLabel("")),)
	// 创建右侧功能栏
    funcPanel := container.NewHBox(
        widget.NewButton("🔊+", func() {
            C.keyPress(touchSession.handle, 24)
        }),
        widget.NewButton("🔊-", func() {
            C.keyPress(touchSession.handle, 25)
        }),
        widget.NewButton("🏠", func() {
            C.keyPress(touchSession.handle, 3)
        }),
        widget.NewButton("↩", func() {
            C.keyPress(touchSession.handle, 4)
        }),
    )
	...
	//touchSession.windowSize = fyne.NewSize(540, 960)
	...
	projectionWindow.SetOnClosed(func() {
		close(stopChan)
		wg.Wait()
		closeDevice(C.long(touchSession.handle))
		touchSession.handle = 0
		<-videoStreamChan
	...
	projectionWindow.Canvas().SetOnTypedKey(func(ev *fyne.KeyEvent) {
		...
		switch ev.Name {
		case fyne.KeyReturn:
			C.keyPress(touchSession.handle, 66)
		case fyne.KeyBackspace:
			C.keyPress(touchSession.handle, 67)
		case fyne.KeyDelete:
			C.keyPress(touchSession.handle, 112)
		case fyne.KeyLeft:
			C.keyPress(touchSession.handle, 21) // KEYCODE_DPAD_LEFT
		case fyne.KeyRight:
			C.keyPress(touchSession.handle, 22) // KEYCODE_DPAD_RIGHT
		// 可根据需要添加更多按键映射
		}
	})
	...
	projectionWindow.Canvas().SetOnTypedRune(func(r rune) {
		...
		C.sendText(touchSession.handle, cstr)
	})
